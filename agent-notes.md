...
- The predicted-price chart line applies display-only exponential smoothing over up to 12 chart points. Raw snapshot values remain the API payload; smoothing exists because PM prices can stay flat across buckets and then jump after trades, which reads like a step line even with Chart.js curve interpolation enabled.
...
- The trader bot loads the nuke.fm root `.env` before reading `OPENROUTER_API_KEY`, and that project env value overrides inherited shell values. This keeps bot OpenRouter spend attributable to nuke.fm even if a parent shell still has an old shared key.
